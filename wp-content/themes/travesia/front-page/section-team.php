<div class="front_page_section front_page_section_team<?php
			$travesia_scheme = travesia_get_theme_option('front_page_team_scheme');
			if (!travesia_is_inherit($travesia_scheme)) echo ' scheme_'.esc_attr($travesia_scheme);
			echo ' front_page_section_paddings_'.esc_attr(travesia_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$travesia_css = '';
		$travesia_bg_image = travesia_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$travesia_anchor_icon = travesia_get_theme_option('front_page_team_anchor_icon');	
	$travesia_anchor_text = travesia_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (travesia_get_theme_option('front_page_team_fullheight'))
				echo ' travesia-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$travesia_css = '';
			$travesia_bg_mask = travesia_get_theme_option('front_page_team_bg_mask');
			$travesia_bg_color = travesia_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$travesia_caption = travesia_get_theme_option('front_page_team_caption');
			if (!empty($travesia_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($travesia_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($travesia_caption); ?></h2><?php
			}
		
			// Description (text)
			$travesia_description = travesia_get_theme_option('front_page_team_description');
			if (!empty($travesia_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($travesia_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($travesia_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!travesia_exists_trx_addons())
						...
						travesia_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
