...
if (!function_exists('travesia_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'travesia_hovers_theme_setup3', 3 );
	function travesia_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		travesia_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'travesia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'travesia') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'travesia'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'travesia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'travesia') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
...
if (!function_exists('travesia_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'travesia_hovers_theme_setup9', 9 );
	function travesia_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'travesia_hovers_frontend_scripts', 1010 );
		add_filter( 'travesia_filter_localize_script','travesia_hovers_localize_script' );
		add_filter( 'travesia_filter_merge_scripts',	'travesia_hovers_merge_scripts' );
		add_filter( 'travesia_filter_merge_styles',	'travesia_hovers_merge_styles' );
		add_filter( 'travesia_filter_get_css', 		'travesia_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'travesia_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'travesia_hovers_frontend_scripts', 1010 );
	function travesia_hovers_frontend_scripts() {
		if ( travesia_is_on(travesia_get_theme_option('debug_mode')) && travesia_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'travesia-hovers', travesia_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( travesia_is_on(travesia_get_theme_option('debug_mode')) && travesia_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'travesia-hovers',  travesia_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('travesia_hovers_merge_scripts')) {
	//Handler of the add_filter( 'travesia_filter_merge_scripts', 'travesia_hovers_merge_scripts' );
	function travesia_hovers_merge_scripts($list) {
...
if (!function_exists('travesia_hovers_merge_styles')) {
	//Handler of the add_filter( 'travesia_filter_merge_styles', 'travesia_hovers_merge_styles' );
	function travesia_hovers_merge_styles($list) {
...
if (!function_exists('travesia_hovers_localize_script')) {
	//Handler of the add_filter( 'travesia_filter_localize_script','travesia_hovers_localize_script' );
	function travesia_hovers_localize_script($arr) {
		$arr['button_hover'] = travesia_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('travesia_hovers_add_icons') ) {
	function travesia_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					travesia_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'travesia') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'travesia');
						else
							esc_html_e('Details', 'travesia');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php esc_url(the_permalink()); ?>" aria-hidden="true" class="icon-right-arrow-2"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'travesia_hovers_get_css' ) ) {
	//Handler of the add_filter( 'travesia_filter_get_css', 'travesia_hovers_get_css', 10, 4 );
	function travesia_hovers_get_css($css, $colors, $fonts, $scheme='') {
