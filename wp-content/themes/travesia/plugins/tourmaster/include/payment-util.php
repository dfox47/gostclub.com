	...
	*/
	...
	if( !function_exists('tourmaster_get_payment_page') ){
		function tourmaster_get_payment_page($booking_detail, $is_single = false){
			...
			if( empty($date_price) ){
				...
				$ret .= esc_html__('An error occurred while processing your request.','travesia');
				if( !empty($booking_detail['tour-id']) ){
					$ret .= '<br><br><a href="' . esc_url(get_permalink($booking_detail['tour-id'])) . '" >' . esc_html__('Back to Tour Page', 'travesia') . '</a>';
				}else{
					$ret .= '<br><br><a href="' . esc_url(home_url('/')) . '" >' . esc_html__('Back to Home Page', 'travesia') . '</a>';
				}
				$ret .= '</div>';
	...
	if( !function_exists('tourmaster_get_booking_bar_summary') ){
		function tourmaster_get_booking_bar_summary( $tour_option, $date_price, $booking_detail, $editable = false ){
			...
			$ret .= '<span class="tourmaster-head" >' . esc_html__('Travel Date', 'travesia') . ' : </span>';
			$ret .= '<span class="tourmaster-tail" >';
			$ret .= tourmaster_date_format($booking_detail['tour-date']);
			if( $editable ){
				$ret .= ' ( <span class="tourmaster-tour-booking-bar-date-edit" >' . esc_html__('edit', 'travesia') . '</span> )';
			...
			if( !empty($booking_detail['package']) ){
				...
				$ret .= '<span class="tourmaster-head" >' . esc_html__('Package', 'travesia') . ' : </span>';
			...
			if( $tour_option['tour-type'] == 'multiple' && !empty($tour_option['multiple-duration']) ){
				...
				$ret .= '<span class="tourmaster-head" >' . esc_html__('End Date', 'travesia') . ' : </span>';
				...
				$ret .= '<span class="tourmaster-head" >' . esc_html__('Period', 'travesia') . ' : </span>';
				$ret .= '<span class="tourmaster-tail" >' . $tour_duration . ' ';
				$ret .= ($tour_option['multiple-duration'] > 1)? esc_html__('Days', 'travesia'): esc_html__('Day', 'travesia');
				$ret .= '</span>';
				$ret .= '</div>';
			}
			...
			// no room based
				...
				if( $date_price['pricing-method'] == 'fixed' ){
					...
					$ret .= '<span class="tourmaster-head" >' . esc_html__('Traveller', 'travesia') . ' : </span>';
				...
				// variable price
					...
					if( !empty($date_price['adult-price']) ){
						...
						$ret .= '<span class="tourmaster-head" >' . esc_html__('Adult', 'travesia') . ' : </span>';
					...
					if( !empty($date_price['children-price']) ){
						...
						$ret .= '<span class="tourmaster-head" >' . esc_html__('Children', 'travesia') . ' : </span>';
					...
					if( !empty($date_price['student-price']) ){
						...
						$ret .= '<span class="tourmaster-head" >' . esc_html__('Student', 'travesia') . ' : </span>';
					...
					if( !empty($date_price['infant-price']) ){
						...
						$ret .= '<span class="tourmaster-head" >' . esc_html__('Infant', 'travesia') . ' : </span>';
			...
			// room based	
				...
				for( $i = 0; $i < $booking_detail['tour-room']; $i++ ){
					...
					$ret .= '<div class="tourmaster-tour-booking-bar-summary-room-text" >' . esc_html__('Room', 'travesia') . ' ' . ($i + 1) . '</div>';
					// fixed price
					if( $date_price['pricing-method'] == 'fixed' ){
						$ret .= '<div class="tourmaster-tour-booking-bar-summary-people-amount" >';
						$ret .= '<span class="tourmaster-head" >' . esc_html__('Traveller', 'travesia') . ' : </span>';
			...
			if( $editable ){
				...
				$ret .= '<input type="text" class="tourmaster-tour-booking-bar-coupon" name="coupon-code" placeholder="' . esc_html__('Coupon Code', 'travesia') . '" ';
				...
				$ret .= ' >' . esc_html__('Apply', 'travesia') . '</a>';
			...
			$ret .= '<span class="tourmaster-tour-booking-bar-price-breakdown-link" id="tourmaster-tour-booking-bar-price-breakdown-link" >' . esc_html__('View Price Breakdown', 'travesia') . '</span>';
			...
			if( $enable_deposit_payment == 'enable' && !empty($deposit_amount) && $editable ){
				...
				$ret .= esc_html__('Pay Full Amount', 'travesia');
				$ret .= '</span>';
				$ret .= '</label>'; 
				...
				$ret .= '<span class="tourmaster-content" >';
				$ret .= '<i class="icon_check_alt2" ></i>';
				$ret .= sprintf(esc_html__('Pay %d%% Deposit', 'travesia'), $deposit_amount);
				$ret .= '</span>';
				$ret .= '</label>';
				$ret .= '</div>';
			}else{
			...
			$ret .= '<span class="tourmaster-tour-booking-bar-total-price-title" >' . esc_html__('Total Price', 'travesia') . '</span>';
			...
			if( $enable_deposit_payment == 'enable' && !empty($deposit_amount) ){
				...
				$ret .= '<span class="tourmaster-tour-booking-bar-deposit-title" >' . sprintf(esc_html__('Pay %d%% deposit','travesia'), $deposit_amount) . '</span>';
				$ret .= '<span class="tourmaster-tour-booking-bar-deposit-price" >' . tourmaster_money_format($deposit_price) . '</span>';
				$ret .= '<span class="tourmaster-tour-booking-bar-deposit-caption" >' . esc_html__('*Pay the rest later', 'travesia') . '</span>';
				$ret .= '</div>';
			}

			if( $editable ){
				$ret .= '<a class="tourmaster-tour-booking-continue tourmaster-button tourmaster-payment-step" data-step="3" >' . esc_html__('Next Step', 'travesia') . '</a>';
			}
	...
	if( !function_exists('tourmaster_payment_service_form') ){
		function tourmaster_payment_service_form( $tour_option, $booking_detail ){
			...
			if( !empty($tour_option['tour-service']) ){
				...
				$ret .= '<h3 class="tourmaster-payment-service-form-title" >' . esc_html__('Please select your preferred additional services.', 'travesia') . '</h3>';
				...
				foreach($tour_option['tour-service'] as $service_id){
					...
					if( !empty($service_option['mandatory']) && $service_option['mandatory'] == 'enable' ){
						$ret .= 'checked onclick="return true;" ';
					}else{
					...
					if( $service_option['per'] == 'person' ){
						$ret .= '<span class="tourmaster-sep" >/</span>' . esc_html__('Person', 'travesia');
						$ret .= '<input type="hidden" name="service-amount[]" value="1" />';
					}else if( $service_option['per'] == 'group' ){
						$ret .= '<span class="tourmaster-sep" >/</span>' . esc_html__('Group', 'travesia');
						$ret .= '<input type="hidden" name="service-amount[]" value="1" />';
					}else if( $service_option['per'] == 'room' ){
						$ret .= '<span class="tourmaster-sep" >/</span>' . esc_html__('Room', 'travesia');
						$ret .= '<input type="hidden" name="service-amount[]" value="1" />';
	...
	if( !function_exists('tourmaster_payment_traveller_title') ){
		function tourmaster_payment_traveller_title(){
			return apply_filters('tourmaster_traveller_title_types', array(
				'mr' => esc_html__('Mr', 'travesia'),
				'mrs' => esc_html__('Mrs', 'travesia'),
				'ms' => esc_html__('Ms', 'travesia'),
				'miss' => esc_html__('Miss', 'travesia'),
				'master' => esc_html__('Master', 'travesia'),
			));
		}
	}
	if( !function_exists('tourmaster_payment_traveller_input') ){
		function tourmaster_payment_traveller_input($tour_option, $booking_detail, $i, $required = true){
			...
			$ret .= '<span class="tourmaster-head">' . esc_html__('Traveller', 'travesia') . ' ' . ($i + 1) . '</span>';
			$ret .= '<span class="tourmaster-tail clearfix">';
			$ret .= $title_html;
			$ret .= '<input type="text" class="tourmaster-traveller-info-input" name="traveller_first_name[]" value="' . esc_attr($first_name) . '" placeholder="' . esc_html__('First Name', 'travesia') . ($required? ' *': '') . '" ' . $data_required . ' />';
			$ret .= '<input type="text" class="tourmaster-traveller-info-input" name="traveller_last_name[]" value="' . esc_attr($last_name) . '" placeholder="' . esc_html__('Last Name', 'travesia') . ($required? ' *': '') . '" ' . $data_required . ' />';
			if( !empty($tour_option['require-traveller-passport']) && $tour_option['require-traveller-passport'] == 'enable' ){
				$ret .= '<input type="text" class="tourmaster-traveller-info-passport" name="traveller_passport[]" value="' . esc_attr($passport) . '" placeholder="' . esc_html__('Passport Number', 'travesia') . ($required? ' *': '') . '" ' . $data_required . ' />';
			}
			$ret .= '</span>';
			$ret .= '</div>';
			...
			if( !empty($tour_option['require-each-traveller-info']) && $tour_option['require-each-traveller-info'] == 'enable' ){
				...
				if( $date_price['pricing-method'] == 'group' ){
					...
					if( $traveller_amount > 0 ){
						...
						$ret .= esc_html__('Traveller Details', 'travesia');
						$ret .= '</h3>';
				...
				for( $i = 0; $i < sizeof($booking_detail['traveller_first_name']); $i++ ){
					if( !empty($booking_detail['traveller_first_name'][$i]) || !empty($booking_detail['traveller_last_name'][$i]) ){
						...
						if( !empty($booking_detail['traveller_passport'][$i]) ){
							$ret .= '<br>' . esc_html__('Passport ID :', 'travesia') . ' ' . $booking_detail['traveller_passport'][$i];
						}
						$ret .= '</span>';
						$ret .= '</div>';
					}
	...
	if( !function_exists('tourmaster_get_payment_contact_form_fields') ){
		function tourmaster_get_payment_contact_form_fields(){
			...
			return array(
				'first_name' => array(
					'title' => esc_html__('First Name', 'travesia'),
					'type' => 'text',
					'required' => true
				),
				'last_name' => array(
					'title' => esc_html__('Last Name', 'travesia'),
					'type' => 'text',
					'required' => true
				),
				'email' => array(
					'title' => esc_html__('Email', 'travesia'),
					'type' => 'text',
					'required' => true
				),
				'phone' => array(
					'title' => esc_html__('Phone', 'travesia'),
					'type' => 'text',
					'required' => true
				),
				'country' => array(
					'title' => esc_html__('Country', 'travesia'),
					'type' => 'combobox',
					'required' => true, 
				...
				'contact_address' => array(
					'title' => esc_html__('Address', 'travesia'),
					'type' => 'textarea'
				),
			);
	...
	if( !function_exists('tourmaster_payment_contact_form') ){
		function tourmaster_payment_contact_form( $booking_detail ){
			...
			$ret .= esc_html__('Contact Details', 'travesia');
			$ret .= '</h3>';
			...
			$ret .= esc_html__('Billing Details', 'travesia');
			$ret .= '</h3>';
			...
			$ret .= '<span class="tourmaster-payment-billing-copy-text" >' . esc_html__('The same as contact details', 'travesia') . '</span>';
			...
			$ret .= esc_html__('Notes', 'travesia');
			$ret .= '</h3>';
			$ret .= '<div class="tourmaster-additional-note-field clearfix">';
			$ret .= '<span class="tourmaster-head">' . esc_html__('Additional Notes', 'travesia') . '</span>';
			$ret .= '<span class="tourmaster-tail clearfix">';
			...
			$ret .= '<div class="tourmaster-tour-booking-required-error tourmaster-notification-box tourmaster-failure">' . esc_html__('Please fill all required fields.', 'travesia') . '</div>';
			$ret .= '<a class="tourmaster-tour-booking-continue tourmaster-button tourmaster-payment-step" data-step="3" >' . esc_html__('Next Step', 'travesia') . '</a>';
	...
	if( !function_exists('tourmaster_payment_method') ){
		function tourmaster_payment_method(){
			...
			$ret .= '<h3 class="tourmaster-payment-method-title" >' . esc_html__('Please select a payment method', 'travesia') . '</h3>';
			...
			if( $paypal_enable || $credit_card_enable ){
				...
				if( $paypal_enable ){
					...
					if( !empty($paypal_button_atts['service-fee']) ){
						...
						$ret .= sprintf(esc_html__('Additional %s%% is charged for PayPal payment.', 'travesia'), $paypal_service_fee);
						$ret .= '</div>';
					}
					$ret .= '</div>';
				}
			...
			if( in_array('booking', $payment_method) ){
				...
				if( sizeof($payment_method) > 1 ){
					...
					$ret .= '<span class="tourmaster-middle" >' . esc_html__('OR', 'travesia') . '</span>';
				...
				if( is_user_logged_in() ){
					...
					$ret .= esc_html__('Book and pay later', 'travesia');
					$ret .= '</a>';
				}else{
					...
					if( $book_by_email == 'enable' ){
						$ret .= '<a class="tourmaster-button tourmaster-payment-method-booking-button tourmaster-payment-step" data-name="payment-method" data-value="booking" data-step="4" >';
						$ret .= esc_html__('Book now via email', 'travesia');
						$ret .= '</a>';
					}else{
						...
						$ret .= tourmaster_lightbox_content(array(
							...
							'title' => esc_html__('To book and pay later requires an account', 'travesia'),
				...
				if( is_user_logged_in() ){
					...
					$ret .= esc_html__('* If you wish to do a bank transfer, please select "Book and pay later" button.', 'travesia'); 
					$ret .= '<br>' . esc_html__('You will have an option to submit payment receipt on your dashboard page.', 'travesia');
					$ret .= '</div>';
				}
			...
			$ret .= sprintf(wp_kses(
				__('* To continue means you\'re okay with our <a href="%s" target="_blank">Terms of Service</a> and <a href="%s" target="_blank">Privacy Statement</a>.', 'travesia'), 
	...
	if( !function_exists('tourmaster_payment_complete') ){
		function tourmaster_payment_complete(){
			...
			$ret .= '<div class="tourmaster-payment-complete-head" >' . esc_html__('Booking Completed!', 'travesia') . '</div>';
			$ret .= '<div class="tourmaster-payment-complete-content-wrap" >';
			$ret .= '<i class=" icon_check_alt2 tourmaster-payment-complete-icon" ></i>';
			$ret .= '<div class="tourmaster-payment-complete-thank-you" >' . esc_html__('Thank you!', 'travesia') . '</div>';
			$ret .= '<div class="tourmaster-payment-complete-content" >';
			$ret .= wp_kses(__('Your booking detail has been sent to your email. <br> You can check the payment status from your dashboard.', 'travesia'), array('br'=>array()));
			$ret .= '</div>'; // tourmaster-payment-complete-content

			if( is_user_logged_in() ){
				$ret .= '<a class="tourmaster-payment-complete-button tourmaster-button" href="' . esc_url(tourmaster_get_template_url('user')) . '" >' . esc_html__('Go to my dashboard', 'travesia') . '</a>';
			}else{
				$ret .= '<a class="tourmaster-payment-complete-button tourmaster-button" href="' . esc_url(home_url("/")) . '" >' . esc_html__('Go to homepage', 'travesia') . '</a>';
			}
	...
	if( !function_exists('tourmaster_payment_complete_delay') ){
		function tourmaster_payment_complete_delay(){
			...
			$ret .= '<div class="tourmaster-payment-complete-thank-you" >' . esc_html__('Thank you!', 'travesia') . '</div>';
			$ret .= '<div class="tourmaster-payment-complete-content" >';
			$ret .= wp_kses(__('Your booking detail will be sent to your email shortly. <br> You can check the payment status from your dashboard.<br> ( There might be some delay processing the paypal payment )', 'travesia'), array('br'=>array()));
			$ret .= '</div>'; // tourmaster-payment-complete-content
		...
		function tourmaster_lb_payment_receipt( $transaction_id ){
			...
			$form_fields = array(
				'receipt' => array(
					'title' => esc_html__('Select Image', 'travesia'),
					'type' => 'file',
				),
				'transaction-id' => array(
					'title' => esc_html__('Transaction ID ( from the receipt )', 'travesia'),
					'type' => 'text',
					'required' => true
				)
			);
			...
			if( $enable_deposit_payment == 'enable' && !empty($deposit_amount) ){
				...
				$ret .= '<div class="tourmaster-head" >' . esc_html__('Select Payment Type', 'travesia') . '</div>';
				...
				$ret .= '<span class="tourmaster-content" >';
				$ret .= '<i class="icon_check_alt2" ></i>';
				$ret .= sprintf(esc_html__('Pay Full Amount : %s', 'travesia'), tourmaster_money_format($pricing_info['total-price']));
				$ret .= '</span>';
				$ret .= '</label>'; 
				...
				$ret .= '<span class="tourmaster-content" >';
				$ret .= '<i class="icon_check_alt2" ></i>';
				$ret .= sprintf(esc_html__('Pay %d%% Deposit : %s', 'travesia'), $deposit_amount, tourmaster_money_format($deposit_price));
			...
			$ret .= esc_html__('Please fill all required fields', 'travesia');
			$ret .= '</div>';

			$ret .= '<div class="tourmaster-payment-receipt-field-submit" >';
			$ret .= '<input class="tourmaster-payment-receipt-field-submit-button tourmaster-button" type="submit" value="' . esc_html__('Submit', 'travesia') . '" />';
			$ret .= '</div>';

			$ret .= '<div class="tourmaster-payment-receipt-description" >';
			$ret .= esc_html__('* Please wait for the verification process after submitting the receipt. This could take up to couple days. You can check the status of submission from your "Dashboard" or "My Booking" page.', 'travesia');
			$ret .= '</div>';
			...
			if( !empty($coupons) ){
				...
				if( !empty($coupon_option['coupon-expiry']) ){
					if( strtotime(date("Y-m-d")) > strtotime($coupon_option['coupon-expiry']) ){
						return array(
							'status' => 'failed',
							'message' => esc_html__('This coupon has been expired, please try again with different coupon', 'travesia')
						);
					}
				}
				...
				if( !empty($coupon_option['apply-to-specific-tour']) ){
					...
					if( !in_array($tour_id, $allow_tours) ){
						return array(
							'status' => 'failed',
							'message' => esc_html__('This coupon is not available for this tour, please try again with different coupon', 'travesia')
						);
					}
				}
				...
				if( !empty($coupon_option['coupon-amount']) ){
					...
					if( $used_coupon >= $coupon_option['coupon-amount'] ){
						return array(
							'status' => 'failed',
							'message' => esc_html__('This coupon has been used up, please try again with different coupon', 'travesia')
						);
					}
				}
				...
				$message = sprintf(esc_html__('You got %s discount', 'travesia'), $discount_amount);
				return array(
					'status' => 'success',
					'message' => $message, 
					'data' => $coupon_option
				...
				return array(
					'status' => 'failed',
					'message' => esc_html__('Invalid coupon code, please try again with different coupon', 'travesia')
				);
			}
		}
	}
		...
		function tourmaster_ajax_validate_coupon_code(){
			...
			if( empty($_POST['coupon_code']) ){
				wp_die(json_encode(array(
					'status' => 'failed',
					'message' => esc_html__('Please fill in the coupon code', 'travesia')
				)));
			}else{
